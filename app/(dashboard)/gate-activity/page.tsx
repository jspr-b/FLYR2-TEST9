      ...
      <div className="lg:ml-64 flex-1 bg-gray-50 min-h-screen">
        <div className="p-3 xs:p-4 sm:p-6 lg:p-8 pt-14 xs:pt-16 lg:pt-8">
          ...
          <div className="grid gap-3 xs:gap-4 sm:gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-2 xs:gap-3 sm:gap-4">
                ...
                <Card className="h-[400px] xs:h-[500px] sm:h-[600px] flex flex-col">
                  <CardHeader className="pb-3 flex-shrink-0">
                    <div className="flex items-center justify-between">
                      <div>
                        <CardTitle className="text-base">Gate Statistics</CardTitle>
                        <p className="text-xs text-gray-600">Gate allocation overview</p>
                      </div>
                      <button
                        onClick={() => refetch(false)}
                  ...
                  <CardContent className="pt-0 flex-1 overflow-hidden flex flex-col">
                    <div className="space-y-3 flex-1">
                      {/* Total Unique Gates Used Today */}
                      <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="text-xs text-blue-700">Total Unique Gates Used Today</div>
                            <div className="text-2xl font-bold text-blue-900">
                              {data?.summary?.actualTotalGatesUsed || gateStatistics?.uniqueGatesUsed || data?.summary?.totalGates || 0}
                            </div>
                          </div>
                          <div className="text-right">
                            <div className="text-xs text-blue-700">Of Schiphol Total</div>
                            <div className="text-lg font-semibold text-blue-800">
                              {Math.round(((data?.summary?.actualTotalGatesUsed || gateStatistics?.uniqueGatesUsed || 0) / 223) * 100)}%
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="text-xs text-green-700">Currently Active Gates</div>
                            <div className="text-2xl font-bold text-green-900">
                              {processedData?.activeGates?.length || 0}
                            </div>
                            <div className="text-[10px] text-green-600 mt-0.5">Gates with aircraft present</div>
                          </div>
                          <div className="text-right">
                            <div className="text-xs text-green-700">Utilization</div>
                            <div className="text-lg font-semibold text-green-800">
                              {data?.summary?.totalGates > 0 
                                ? Math.round((processedData?.activeGates?.length || 0) / data.summary.totalGates * 100)
                                : 0}%
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="space-y-2">
                        <div className="text-xs font-medium text-gray-700">Flight Gate Allocation:</div>
                        
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Flights with Gates Assigned</span>
                            <span className="font-semibold">
                              {data?.summary?.actualFlightsWithGates || gateStatistics?.assignedGates || 0}
                            </span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-1">
                            <div 
                              className="bg-blue-500 h-1 rounded-full transition-all duration-500"
                              style={{ width: `${totalFlights > 0 ? ((data?.summary?.actualFlightsWithGates || gateStatistics?.assignedGates || 0) / totalFlights * 100) : 0}%` }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">Gates To Be Determined (TBD)</span>
                            <span className="font-semibold text-amber-600">
                              {gateStatistics?.tbdGates || 0}
                            </span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-1">
                            <div 
                              className="bg-amber-500 h-1 rounded-full transition-all duration-500"
                              style={{ width: `${totalFlights > 0 ? ((gateStatistics?.tbdGates || 0) / totalFlights * 100) : 0}%` }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="font-semibold text-red-600">
                              {gateStatistics?.noGates || 0}
                            </span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-1">
                            <div 
                              className="bg-red-500 h-1 rounded-full transition-all duration-500"
                              style={{ width: `${totalFlights > 0 ? ((gateStatistics?.noGates || 0) / totalFlights * 100) : 0}%` }}
                            />
                          </div>
                        </div>

                        {gateStatistics?.cancelledWithGates > 0 && (
                          <div className="mt-2 pt-2 border-t border-gray-200">
                            <div className="text-[10px] text-gray-500">
                              * Includes {gateStatistics.cancelledWithGates} cancelled flights that had gate assignments
                            </div>
                          </div>
                        )}
                      </div>
              ...
              {/* Gate Type Distribution - Bus gates vs jet bridges */}
              <div className="xl:col-span-1">
