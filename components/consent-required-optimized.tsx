...
interface ConsentRequiredOptimizedProps {
  children: React.ReactNode
  onDataPreload?: () => Promise<void>
}

export function ConsentRequiredOptimized({ children, onDataPreload }: ConsentRequiredOptimizedProps) {
  const [isLoading, setIsLoading] = useState(false)
  const [dataPreloaded, setDataPreloaded] = useState(false)
  const router = useRouter()

  // Start preloading data as soon as component mounts
  useEffect(() => {
    const preloadData = async () => {
      try {
        // Start loading dashboard data in the background
        const endpoints = [
          '/api/dashboard/kpis?includeCancelled=true',
          '/api/aircraft/performance',
          '/api/gate-occupancy',
          '/api/delay-trends/hourly'
        ]

        // Initiate all requests in parallel but don't await them
        // This warms up the API cache without blocking the UI
        const promises = endpoints.map(endpoint => 
          fetch(endpoint).catch(err => console.error(`Preload failed for ${endpoint}:`, err))
        )

        // Call custom preload function if provided
        if (onDataPreload) {
          promises.push(onDataPreload())
        }
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Since data is already preloading/preloaded, the refresh will be fast
        router.refresh()
      } else {
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4">
        <Card className={cn(
          ...
          <CardContent className="space-y-8 pb-8">
            ...
            {!dataPreloaded && (
              <div className="flex items-center justify-center gap-2 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
