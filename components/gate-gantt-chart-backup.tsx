...
export function GateGanttChart({ gateData }: GateGanttChartProps) {
  ...
  // Fixed time range from 05:00 to 00:00 (next day) in Amsterdam time
  const { dynamicStartTime, dynamicEndTime } = useMemo(() => {
    ...
    // Create fixed time range for today in Amsterdam time
    const now = getCurrentAmsterdamTime()
    const start = new Date(now)
    ...
    console.log('Fixed timeline range:', {
  ...
  const getFlightBarStyle = (flight: any, usePixels: boolean = false) => {
    const timeline = getFlightTimeline(flight)
    ...
    if (usePixels) {
      // Pixel-based positioning for new scrollable layout
      const pixelsPerMs = (timeSlots.length * 120) / totalViewDuration
      const leftPixels = Math.max(0, flightStartOffset * pixelsPerMs)
      const rightPixels = Math.min(timeSlots.length * 120, flightEndOffset * pixelsPerMs)
      const widthPixels = rightPixels - leftPixels
      
      return {
        left: `${leftPixels}px`,
        width: `${Math.max(widthPixels, 80)}px`, // Minimum 80px for better visibility
        minWidth: '80px'
      }
    } else {
      // Percentage-based positioning for fullscreen modal
  ...
  const getCurrentTimePosition = (usePixels: boolean = false) => {
    ...
    if (usePixels) {
      ...
      const position = (intervalIndex + percentageInInterval) * 120 // 120px per interval
  ...
  const renderGanttContent = (isFullScreen = false) => {
    const timelineWidth = timeSlots.length * 120 // 120px per time slot for better spacing
      ...
      return (
        <div className="w-full h-full overflow-auto" ref={modalContentScrollRef}>
          <div className="w-full">
            ...
            <div className="sticky top-0 bg-white z-10 border-b border-gray-200">
              <div className="flex h-8 relative">
                ...
                {timeSlots.slice(0, -1).map((_, index) => (
                  <div key={index} className="flex-1 bg-gray-50 border-l border-gray-200" />
                ))}
            ...
            {/* Gates Content */}
            <div className="w-full">{processedGateData.map((gate, index) => (
              <div key={gate.gateID} className="border-b border-gray-100 last:border-b-0">
                ...
                >
                  {/* Grid Background - Pixel-based positioning */}
                  ...
                  <div className="absolute inset-0">
                    {((gate as any).flightStacks || []).map((stack: any[], stackIndex: number) => (
                      ...
                      {stack.map((flight, flightIndex) => {
                        ...
                        // For main view (pixel-based), calculate pixel positions relative to flight bar
                        const isMainView = !isFullScreen
                        let scheduledDeparturePosition, actualDeparturePosition
                        
                        if (isMainView) {
                          // Pixel positions relative to flight bar start
                          const pixelsPerMs = getFlightBarStyle(flight, true).width.replace('px', '') / barDuration
                          scheduledDeparturePosition = scheduledDepartureOffset * parseFloat(pixelsPerMs)
                          actualDeparturePosition = actualDepartureOffset * parseFloat(pixelsPerMs)
                        } else {
                          // Percentage positions for modal
                          scheduledDeparturePosition = (scheduledDepartureOffset / barDuration) * 100
                          actualDeparturePosition = (actualDepartureOffset / barDuration) * 100
                        }
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...getFlightBarStyle(flight, true), // Use pixel-based positioning
                              top: `${barTop}px`,
                              height: `${barHeight}px`
                            }}
                            ...
                            {timeline.isTimelineShifted ? (
                              <>
                                ...
                                <div 
                                  ...
                                  style={{ 
                                    left: isMainView 
                                      ? `${Math.max(0, scheduledDeparturePosition)}px` 
                                      : `${Math.max(0, Math.min(100, scheduledDeparturePosition))}%` 
                                  }}
                                ...
                                <div 
                                  ...
                                  style={{ 
                                    left: isMainView 
                                      ? `${Math.max(0, actualDeparturePosition)}px` 
                                      : `${Math.max(0, Math.min(100, actualDeparturePosition))}%` 
                                  }}
                    ...
                    {(() => {
                      const { position, isVisible } = getCurrentTimePosition(true) // Use pixel-based positioning
                      ...
                      return (
                        <div
                          ...
                          style={{ left: `${position}px`, transform: 'translateX(-0.5px)' }}
                        />
                      )
                    })()}
                  </div>
                </div>
                {/* Spacing divider between gates */}
                {index < processedGateData.length - 1 && (
                  <div className="h-2 bg-gray-100 border-t border-b border-gray-200" style={{ minWidth: `${timelineWidth + 132}px` }} />
                )}
              </div>
            ))}
    ...
    return (
      <div className="w-full h-full flex flex-col">
        ...
        <div className="sticky top-0 bg-white z-20 border-b border-gray-200 shadow-sm">
          <div className="flex">
            {/* Fixed space for gate labels column */}
            <div className="w-32 flex-shrink-0 bg-gray-50 border-r border-gray-200">
              <div className="h-8"></div>
            </div>
            
            {/* Scrollable time header */}
            <div ref={headerScrollRef} className="flex-1 overflow-x-auto scrollbar-hide">
              <div className="relative h-8" style={{ width: `${timelineWidth}px` }}>
                {/* Time slot intervals */}
                {timeSlots.slice(0, -1).map((_, index) => (
                  <div 
                    key={index} 
                    className="absolute h-full bg-gray-50 border-l border-gray-200" 
                    style={{ 
                      left: `${index * 120}px`,
                      width: '120px'
                    }}
        ...
        <div ref={contentScrollRef} className="flex-1 overflow-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
          <div style={{ minWidth: `${timelineWidth + 132}px` }}>
            {processedGateData.map((gate, index) => (
              <div key={gate.gateID} className="group">
                <div className="flex relative">
                  {/* Sticky gate label on the left */}
                  <div className="sticky left-0 z-10 bg-gray-50 border-r border-gray-200 shadow-sm flex-shrink-0 w-32">
                    <div className="flex flex-col justify-center h-full py-2 px-3" style={{ 
                      minHeight: `${Math.max(48, ((gate as any).flightStacks?.length || 0) * 36 + 12)}px`
                    }}>
                  ...
                  {/* Timeline Bar - Scrollable content */}
                  <div 
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-gray-600">
              <span>Timeline:</span>
              <span className="font-medium">05:00 - 00:00 (Full Day)</span>
            </div>
          </div>
        </div>
                  ...
                  <div key={gate.gateID} className="p-2 sm:p-3 md:p-4">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-xs sm:text-sm">{gate.gateID}</span>
                        <span className="text-[10px] sm:text-xs text-gray-500">Pier {gate.pier}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] sm:text-xs text-gray-600">{gate.flights.length} flights</span>
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-1 sm:gap-2">
                                ...
                                <span className="opacity-90 truncate max-w-[80px] sm:max-w-[120px]">{flight.destination}</span>
                              </div>
                              {flight.isDelayed && (
          ...
          <div className="bg-white rounded-lg shadow-xl w-full h-full max-w-none max-h-none flex flex-col">
            <div className="flex items-center justify-between p-6 border-b bg-white">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <span>Timeline:</span>
                  <span className="font-medium">05:00 - 00:00 (Full Day AMS)</span>
                </div>
